...
function normalizeRecordsPage(
  payload: RecordOut[] | RecordsPayload,
  ...
  return {
    ...
    limit: payload.limit ?? params.limit ?? data.length,
    data,
  };
}
...
export async function getCandidates(
  ...
  const pageData = await getCandidatesPage(params);
...
export async function getCandidatesPage(
  ...
  const payload = await apiFetch<RecordOut[] | RecordsPayload>(
    `/records${qs}`
  );

  return normalizeRecordsPage(payload, params);
}
